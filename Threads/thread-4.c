/*
*  thread-4.c - Demostración de la ejecución de un hilo en espacio de kernel
*  se usa la función kthread_create() y wake_up_process()
*  Ligando un hilo a un solo procesador
*/
...
static int hilo_kernel( void *arg )
{
	...
	while( !kthread_should_stop() )//Esperar mientras no se mande a llamar la otra función kthread_stop, mientras no lo detenga
	{
		...
		cpu = smp_processor_id(); //trae el procesador que ejecuta el hilo
...
static int __init funcion_inicio(void)
{
	...
	printk(KERN_INFO "Iniciando el ejemplo 4 de hilos de kenerl \n");
...
static void __exit funcion_exit(void)
{
	...
	printk(KERN_INFO "Terminando la ejecución del ejemplo 4 de hilos \n");
