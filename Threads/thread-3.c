/*
*  thread-3.c - Demostración de la ejecución de un hilo en espacio de kernel
*  se usa la función kthread_run() y wake_up_process()
*/
...
static int hilo_kernel( void *arg )
{
	...
	while( !kthread_should_stop() )//Esperar mientras no se mande a llamar la otra función kthread_stop, mientras no lo detenga
	{
		...
		ssleep(1);
		if( irq == 10 )
			printk(KERN_INFO "Irq detectada: %d \n", cont++ );
...
static int __init funcion_inicio(void)
{
	...
	printk(KERN_INFO "Iniciando el ejemplo 3 de hilos de kenerl \n");
...
static void __exit funcion_exit(void)
{
	...
	printk(KERN_INFO "Terminando la ejecución del ejemplo 3 de hilos \n");
